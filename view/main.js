...
(async () => {
  ...
  canvas.width = mapWidth * squareSize + mapWidth * roadSize;
  canvas.height = mapHeight * squareSize + mapHeight * roadSize;
  ...
  async function render(viewMode, highlightX, highlightY) {
    ...
    for (let x = 1; x <= mapWidth; x++) {
      for (let y = 1; y <= mapHeight; y++) {
        const coordX = x * squareSize + x * roadSize - squareSize + roadSize;
        const coordY = squareSize * mapHeight + roadSize * mapHeight - (y * squareSize + y * roadSize);
